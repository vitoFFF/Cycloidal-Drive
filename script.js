...
function drawHole() {
    ...
    hole.setAttribute('cx', svg.width.baseVal.value / 2 + eccentricity); // Move the center hole to the left by the distance of the eccentricity
    hole.setAttribute('cy', svg.height.baseVal.value / 2);
    hole.setAttribute('r', holeSize); // Use the selected hole size
    hole.setAttribute('fill', '#676469');
...
function drawOutHole() {
    ...
    for (let i = 0; i < numHoles; i++) {
        ...
        const holeX = svg.width.baseVal.value / 2 + ( holeSize + 100) * Math.cos(angle) + eccentricity; // Add some space between the center and output holes
        const holeY = svg.height.baseVal.value / 2 + (holeSize + 100) * Math.sin(angle); // Add some space between the center and output holes
        drawHoleCircle(holeX, holeY, holeRadius);
    }
}
...
function rollerPins() {
    ...
    for (let i = 0; i < numHoles; i++) {
        ...
        const holeX = svg.width.baseVal.value / 2 + ( holeSize + 100) * Math.cos(angle) - eccentricity; // Add some space between the center and output holes
        const holeY = svg.height.baseVal.value / 2 + (holeSize + 100) * Math.sin(angle); // Add some space between the center and output holes
        rollerPinsCircle(holeX, holeY, pinRadius);
    }
}

function rollerPinsCircle(cx, cy, radius) {
    const pinRadius = holeSize - (2 * e); // Use the selected hole size
...
function draw() {
    ...
    drawHole();
    drawOutHole(); // Draw holes around the center hole
    rollerPins();
...
eInput.addEventListener('input', () => {
    ...
    if (e >= R / n) {
        ...
        eValue.textContent = e.toFixed(2); // Update the displayed value
...
holeSizeInput.addEventListener('input', () => {
    ...
    holeSizeValue.textContent = holeSize;
    draw();
});
